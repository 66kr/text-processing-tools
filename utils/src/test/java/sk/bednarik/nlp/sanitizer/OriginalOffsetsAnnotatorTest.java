 ...
 *
 */
@ContextConfiguration(classes = {SanitizerServiceConfig.class})
@RunWith(SpringRunner.class)
public class OriginalOffsetsAnnotatorTest {

  @Autowired
  private SanitizerService sanitizerService;

  @Test
  public void testOriginalOffsets() {
    String input = "\n" +
    ...
    AnnotationPipeline pipeline = sanitizerService.buildSanitizePipelien();
    pipeline.annotate(annotation);

    System.out.println(annotation.get(CoreAnnotations.OriginalTextAnnotation.class).substring(928, 936));
    System.out.println(annotation.get(CoreAnnotations.OriginalTextAnnotation.class).substring(1480, 1523));
    for (CoreLabel token : annotation.get(CoreAnnotations.TokensAnnotation.class)) {
      String noSpaces = annotation.get(CoreAnnotations.TextAnnotation.class)
          .substring(token.beginPosition(), token.endPosition());
      String spaces = annotation.get(CoreAnnotations.OriginalTextAnnotation.class)
          .substring(token.get(AsurAnnotations.OriginalCharacterOffsetBeginAnnotation.class),
              token.get(AsurAnnotations.OriginalCharacterOffsetEndAnnotation.class));
      System.out.println(spaces);
      System.out.println(noSpaces);
      Assert.assertEquals(noSpaces, spaces.replaceAll("[\\s\\p{Z}]", ""));
    }
  }
}